...
const Bookings: React.FC<BookingsProps> = ({ showToast }) => {
    ...
    const [filter, setFilter] = useState<BookingStatus | 'All' | 'Unassigned'>('All');
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const [bookingsData, photographersData, sessionsData] = await Promise.all([
    ...
    const kpiData = useMemo(() => {
        ...
        return {
            ...
            confirmedToday: bookings.filter(b => b.status === 'Confirmed' && b.bookingDate === today).length,
            upcoming: bookings.filter(b => b.bookingDate >= today && b.status !== 'Cancelled').length,
        };
    }, [bookings]);
    ...
    const handleQuickAssign = async (bookingId: string, photographerId: number) => {
        const bookingToUpdate = bookings.find(b => b.id === bookingId);
        if (!bookingToUpdate) return;
        
        const originalBookings = [...bookings];
        const updatedBooking = { ...bookingToUpdate, photographerId, status: 'Confirmed' as BookingStatus };
    ...
    const handleSaveBooking = async (booking: Booking) => {
        ...
        if (isNew) {
            ...
            const tempBooking = { ...booking, id: booking.id || `temp-${Date.now()}` };
        ...
        try {
            ...
            if (isNew) {
                setBookings(prev => prev.map(b => b.id === booking.id ? savedBooking : b));
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-full min-h-[400px]">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Error Loading Bookings</h3>
    ...
    const filterOptions: Array<BookingStatus | 'All' | 'Unassigned'> = ['All', 'Pending', 'Unassigned', 'Confirmed', 'Cancelled'];

    return (
        <div className="animate-fadeIn pb-8">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white">Photo Session Bookings</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Schedule sessions, assign photographers, and manage appointments</p>
                </div>
                <button 
                        ...
                        <table className="w-full text-left min-w-[720px]">
                            ...
                            <tbody>
                                {filteredBookings.length > 0 ? filteredBookings.map(booking => {
                                    ...
                                    >
                                        <td className="p-4">
                                            <p className="font-semibold text-slate-900 dark:text-white">{booking.clientName || 'N/A'}</p>
                                            {booking.clientEmail && (
                                                <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[200px]" title={booking.clientEmail}>{booking.clientEmail}</p>
                                            )}
                                        </td>
                                        <td className="p-4">
                                            {bookingDate ? (
                                                <div>
                                                    <p className="font-medium text-sm">{bookingDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                                                    {booking.bookingTime && (
                                                        <p className="text-xs text-slate-500 dark:text-slate-400">{booking.bookingTime}</p>
                                                    )}
                                                </div>
                                            ) : (
                                        ...
                                        <td className="p-4">
                                            <span className="text-sm font-medium">{session?.name || 'N/A'}</span>
                                        </td>
                                        <td className="p-4" onClick={(e) => e.stopPropagation()}>
                                            {photographer ? (
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => {
                                                        e.stopPropagation();
                                                        handleQuickAssign(booking.id, Number(e.target.value));
                                                    }}
                                        ...
                                        <td className="p-4">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1 ${
                                                ...
                                                {booking.status === 'Pending' && <span className="w-1.5 h-1.5 bg-current rounded-full animate-pulse"></span>}
                                                {booking.status}
                                            </span>
                                        </td>
                                        ...
                                        <td colSpan={6} className="p-12 text-center">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">
                                                    {debouncedSearchTerm ? "No Bookings Found" : filter !== 'All' ? `No ${filter} Bookings` : "No Bookings Yet"}
                                                </h3>
            ...
            {isModalOpen && (
                <BookingEditModal 
                    isOpen={isModalOpen}
                    onClose={() => { setIsModalOpen(false); setBookingToEdit(null); }}
