...
interface AddAdjustmentModalProps {
    ...
    preselectedPhotographerId?: number;
}

const AddAdjustmentModal: React.FC<AddAdjustmentModalProps> = ({ isOpen, onClose, onSave, photographers, preselectedPhotographerId }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} title="Add New Payroll Adjustment">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                     <div>
                        ...
                        <select value={type} onChange={e => setType(e.target.value as 'Bonus' | 'Deduction')} className={inputStyles} required>
                     ...
                     <div>
                        ...
                        <input type="number" value={amount} onChange={e => setAmount(e.target.value === '' ? '' : Number(e.target.value))} placeholder="0.00" className={inputStyles} required />
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g., Exceeded monthly target" className={inputStyles} required />
                </div>
                 <div className="pt-4 flex justify-end space-x-3 border-t border-slate-200 dark:border-slate-700">
                    <button type="button" onClick={handleClose} className="bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-800 dark:text-white font-semibold py-2 px-4 rounded-lg">Cancel</button>
                    <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg">Save Adjustment</button>
                </div>
            </form>
        </Modal>
    );
