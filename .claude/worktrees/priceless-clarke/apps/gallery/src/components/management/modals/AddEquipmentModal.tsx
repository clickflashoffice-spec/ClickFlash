...
import { Equipment, Photographer, EquipmentStatus, EquipmentCategory } from '../../../types';
...
const AddEquipmentModal: React.FC<AddEquipmentModalProps> = ({ isOpen, onClose, onSave, equipmentToEdit, photographers, equipmentCategories }) => {
    const isNew = !equipmentToEdit;
    const [equipment, setEquipment] = useState<Partial<Equipment>>(
        equipmentToEdit || { name: '', type: equipmentCategories[0]?.id || '', status: 'Available' }
    );
    ...
    useEffect(() => {
        if (isOpen) {
            setEquipment(equipmentToEdit || { name: '', type: equipmentCategories[0]?.id || '', status: 'Available' });
        }
    ...
    const handlePhotographerChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setEquipment(prev => ({ ...prev, assignedToPhotographerId: value ? Number(value) : undefined }));
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={isNew ? "Add Equipment" : "Edit Equipment"}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label className={labelStyles}>Equipment Name</label>
                    <input type="text" name="name" value={equipment.name || ''} onChange={handleChange} required className={inputStyles} />
                </div>

                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className={labelStyles}>Type</label>
                        <select name="type" value={equipment.type} onChange={handleChange} required className={inputStyles}>
                            {equipmentCategories.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className={labelStyles}>Status</label>
                        <select name="status" value={equipment.status} onChange={handleChange} required className={inputStyles}>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="destinationId" value={equipment.destinationId || ''} onChange={handleChange} className={inputStyles}>
                            <option value="">Company-wide</option>
                            {MOCK_DESTINATIONS.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className={labelStyles}>Assigned To</label>
                        <select value={equipment.assignedToPhotographerId || ''} onChange={handlePhotographerChange} className={inputStyles}>
                ...
                <div className="pt-4 flex justify-end space-x-3 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg">Save Equipment</button>
                </div>
            </form>
        </Modal>
    );
