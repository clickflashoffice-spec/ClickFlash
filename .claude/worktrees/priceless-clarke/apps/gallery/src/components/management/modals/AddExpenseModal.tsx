...
import { Expense, Photographer, Destination, ExpenseCategory } from '../../../types.ts';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                     ...
                     <div>
                        ...
                        <input type="number" value={cost} onChange={e => setCost(e.target.value === '' ? '' : Number(e.target.value))} placeholder="0.00" autoComplete="off" className={inputStyles} required />
                    </div>
                </div>
                <div>
                    <label className={labelStyles}>Description</label>
                    <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g., New camera lens" autoComplete="off" className={inputStyles} required />
                </div>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className={labelStyles}>Destination</label>
                        <select value={destinationId} onChange={e => setDestinationId(e.target.value)} className={inputStyles} required>
                            {destinations.map(dest => <option key={dest.id} value={dest.id}>{dest.name}</option>)}
                        </select>
                    </div>
                     <div>
                        <label className={labelStyles}>Assign to Photographer (Optional)</label>
                        <select value={photographerId || ''} onChange={e => setPhotographerId(Number(e.target.value) || undefined)} className={inputStyles}>
                 ...
                 <div>
                    ...
                    <select value={category} onChange={e => setCategory(e.target.value)} className={inputStyles} required>
                 ...
                 <div className="pt-4 flex justify-end space-x-3 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg">Save Expense</button>
                </div>
            </form>
        </Modal>
    );
