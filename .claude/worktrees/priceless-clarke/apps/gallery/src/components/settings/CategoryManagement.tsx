...
import CategoryEditModal from '../modals/CategoryEditModal.tsx';
    ...
    return (
        <Card>
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={openAdd} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg">
                    Add Category
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {categories.map((cat, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
                        <span className="font-semibold">{cat}</span>
                        <div className="space-x-2">
                            <button onClick={() => openEdit(cat, index)} className="text-blue-500 hover:text-blue-400 text-sm font-bold px-2">Edit</button>
                            <button onClick={() => handleDelete(index)} className="text-red-500 hover:text-red-400 text-sm font-bold px-2">Delete</button>
                        </div>
                    </div>
                ))}
            </div>
