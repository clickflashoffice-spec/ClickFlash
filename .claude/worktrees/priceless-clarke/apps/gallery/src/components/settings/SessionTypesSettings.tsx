    ...
    const handleSave = async (session: Omit<SessionType, 'id'> | SessionType) => {
        try {
            ...
            if ('id' in session) {
                ...
                await apiService.createSessionType(sessionData as Omit<SessionType, 'id'>);
            }
            ...
            alert(`Failed to save session type: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    };
    
    const handleDelete = async (id: string, name: string) => {
        if (window.confirm(`Are you sure you want to delete the session type "${name}"?`)) {
        ...
        <Card>
            <div className="p-4 mb-6 bg-blue-500/10 border-l-4 border-blue-500 rounded-r-lg">
                <p className="text-blue-800 dark:text-blue-300">
                    <strong>Note:</strong> These are global settings that can be managed from the Management Portal. Any changes made here may be overwritten during a cloud sync. It is recommended to manage session types centrally.
                </p>
            </div>
                    ...
                    <tbody>
                        {sessionTypes.map(s => (
                            <tr key={s.id} className="border-b border-slate-200 dark:border-slate-700/50">
                                ...
                                <td className="p-4 text-right font-mono">{formatCurrency(s.price)}</td>
                                <td className="p-4 text-center space-x-4">
                                    <button onClick={() => handleOpenModal(s)} className="text-blue-500 hover:text-blue-400 font-semibold">Edit</button>
                                    <button onClick={() => handleDelete(s.id, s.name)} className="text-red-500 hover:text-red-400 font-semibold">Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
