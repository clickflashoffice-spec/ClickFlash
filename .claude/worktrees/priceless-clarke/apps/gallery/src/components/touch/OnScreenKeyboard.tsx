...
const Key: React.FC<{ def: KeyDef; onClick: (value: string) => void }> = ({ def, onClick }) => (
    <button
        ...
        style={{ flexGrow: def.grow || 1 }}
        className={`h-14 flex items-center justify-center rounded-lg text-lg font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-500 ${def.className}`}
    >
        {def.display}
    </button>
);
        ...
        <div className="w-full p-2 bg-slate-100 dark:bg-slate-900 rounded-lg space-y-2">
            {currentLayout.map((row, i) => (
                <div key={i} className="flex justify-center space-x-1.5">
                    ...
                    {layout === 'alpha' && i === 1 && <div style={{flexGrow: 0.5}}></div>}
                    {row.map((keyDef, j) => (
                        <Key 
                            key={`${keyDef.value}-${j}`}
