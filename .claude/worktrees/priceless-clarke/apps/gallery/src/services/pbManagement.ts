...
async function cloudApiCall(cloudUrl: string, token: string, method: string, endpoint: string, data?: unknown) {
...
export const pbManagement = {
        ...
        const collections = [
            {
                ...
                schema: [
                    ...
                    { name: 'status', type: 'select', options: { values: ['Draft', 'Finalized', 'Archived'] } }
                ],
            ...
            },
            {
                ...
                schema: [
                    ...
                    { name: 'album', type: 'relation', collectionId: 'albums', cascadeDelete: true },
                    { name: 'url', type: 'file', options: { mimeTypes: ['image/jpeg', 'image/png'] } }
                ],
                listRule: '', viewRule: '',
            },
            {
                ...
                listRule: '', createRule: '', updateRule: '',
            },
            {
                name: 'products',
                type: 'base',
            ...
            },
            {
                 ...
                 listRule: '', createRule: '', updateRule: '', deleteRule: ''
            },
            {
                 name: 'bookings',
                 type: 'base',
     ...
     */
    async syncLocalToCloud(cloudUrl: string, apiKey: string, onProgress: (msg: string, percentage: number) => void) {
        ...
        if (localUsers.length > 0) {
            for (const user of localUsers) {
                try {
                     ...
                     const existingResponse = await fetch(`${cloudUrl}/api/collections/users/records?filter=email="${user.email}"`, {
                         headers: { 'Authorization': `Bearer ${authToken}` }
                     });
                     const existingData = await existingResponse.json();
                     
                     if (existingData.items && existingData.items.length > 0) {
                         await cloudApiCall(cloudUrl, authToken, 'PATCH', `/api/collections/users/records/${existingData.items[0].id}`, {
                             name: user.name,
                             role: user.role,
                         ...
                         await cloudApiCall(cloudUrl, authToken, 'POST', '/api/collections/users/records', {
                            name: user.name,
                            role: user.role,
                            email: user.email,
                            workingHours: user.workingHours,
                    ...
                    logger.warn(`Failed to sync user`, { userName: user.name, error: e instanceof Error ? e.message : String(e) });
                }
                processed++;
                onProgress(`Syncing Users (${processed}/${localUsers.length})...`, 10 + Math.floor((processed / localUsers.length) * 20));
            }
        }
        ...
        if (localOrders.length > 0) {
            for (const order of localOrders) {
                 try {
                     try {
                         await cloudApiCall(cloudUrl, authToken, 'PATCH', `/api/collections/orders/records/${order.id}`, {
                             status: order.status,
                             total: order.total,
                         ...
                         if (error.status === 404) {
                            await cloudApiCall(cloudUrl, authToken, 'POST', '/api/collections/orders/records', {
                                id: order.id, 
                                clientName: order.clientName,
                                email: order.email,
                                total: order.total,
                     ...
                     logger.warn(`Failed to sync order`, { orderId: order.id, error: e instanceof Error ? e.message : String(e) });
                 }
                 processed++;
                 const progress = 30 + Math.floor((processed / localOrders.length) * 40); 
                 onProgress(`Syncing Orders (${processed}/${localOrders.length})...`, progress);
            }
        }
        ...
        for (const exp of localExpenses) {
            try { 
                 if (exp.id) {
                     try { 
                         await cloudApiCall(cloudUrl, authToken, 'PATCH', `/api/collections/expenses/records/${exp.id}`, exp);
                     } catch { 
                         await cloudApiCall(cloudUrl, authToken, 'POST', '/api/collections/expenses/records', exp);
                     }
                 } else { 
                     await cloudApiCall(cloudUrl, authToken, 'POST', '/api/collections/expenses/records', exp);
                 }
            } catch (e) {
                logger.warn(`Failed to sync expense`, { expenseId: exp.id, error: e instanceof Error ? e.message : String(e) });
            }
        }
        ...
        for (const adj of localAdjustments) {
            try { 
                 if (adj.id) {
                     try { 
                         await cloudApiCall(cloudUrl, authToken, 'PATCH', `/api/collections/adjustments/records/${adj.id}`, adj);
                     } catch { 
                         await cloudApiCall(cloudUrl, authToken, 'POST', '/api/collections/adjustments/records', adj);
                     }
                 } else { 
                     await cloudApiCall(cloudUrl, authToken, 'POST', '/api/collections/adjustments/records', adj);
                 }
            } catch (e) {
                logger.warn(`Failed to sync adjustment`, { adjustmentId: adj.id, error: e instanceof Error ? e.message : String(e) });
            }
        }
        ...
        if (sampleUser && sampleUser.destinationId) {
             try {
                 ...
                 const updatedDest = await cloudApiCall(cloudUrl, authToken, 'PATCH', `/api/collections/destinations/records/${sampleUser.destinationId}`, {
                     name: "Online (Synced Just Now)" 
                 });
                 
                 // Reverse Sync: Pull Feature Flags from Cloud to Local
                 if (updatedDest.features) {
                     const localDest = await apiService.getDestinations().then(ds => ds.find(d => d.id === sampleUser.destinationId));
                     if (localDest) {
                         await apiService.updateDestination(localDest.id, {
                             features: updatedDest.features
                         });
                         logger.info("Updated local features from cloud", { features: updatedDest.features });
                     }
                 }
             } catch (e) {
    ...
    async syncLocalToRemote(onProgress: (msg: string, percent: number) => void) {
         onProgress("Feature not fully implemented for Local-to-Remote migration.", 100);
    }
};
