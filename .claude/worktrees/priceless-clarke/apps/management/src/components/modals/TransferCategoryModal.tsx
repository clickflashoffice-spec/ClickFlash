...
const TransferCategoryModal: React.FC<TransferCategoryModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  availableCategories,
  ...
  const [toCategory, setToCategory] = useState(
    availableCategories[1] || availableCategories[0],
  );
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            id="from-category"
            value={fromCategory}
            onChange={(e) => setFromCategory(e.target.value as PhotoCategory)}
        ...
        <div>
          ...
          <select
            id="to-category"
            value={toCategory}
            onChange={(e) => setToCategory(e.target.value as PhotoCategory)}
            className={selectStyle}
            required
          >
            {availableCategories.map((c) => (
        ...
        <div className="pt-4 flex justify-end space-x-3 border-t border-slate-200 dark:border-slate-700">
          <button
            type="button"
            onClick={onClose}
            className="bg-slate-200 hover:bg-slate-300 dark:bg-slate-600 dark:hover:bg-slate-500 text-slate-800 dark:text-white font-semibold py-2 px-4 rounded-lg transition-colors"
          >
            Cancel
          </button>
          <button
