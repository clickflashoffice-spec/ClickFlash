...
export class TokenRefreshService {
    constructor(_dbManager: unknown, _auditLogger: unknown) {}
    generate(userId: string | number, expiresInDays?: number) { return generateRefreshToken(userId, expiresInDays); }
    validateAndConsume(token: string) { return validateAndConsumeRefreshToken(token); }
    revoke(token: string) { revokeRefreshToken(token); }
    revokeUser(userId: string | number) { revokeUserRefreshTokens(userId); }
}
