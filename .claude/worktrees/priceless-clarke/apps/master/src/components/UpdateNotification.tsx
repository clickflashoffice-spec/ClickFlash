...
export const UpdateNotification: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Store cleanup functions returned by preload's on() to prevent listener
    // leaks during prolonged kiosk uptime (React hot-reloads re-mount this).
    ...
    cleanups.push(
      ipc.on('updater:available', (_: unknown, info: { version: string; releaseNotes?: string }) => {
        setStatus(prev => ({
          ...prev,
          checking: false,
          available: true,
    ...
    cleanups.push(
      ipc.on('updater:progress', (_: unknown, progress: { percent: number }) => {
    ...
    cleanups.push(
      ipc.on('updater:error', (_: unknown, error: { message: string }) => {
