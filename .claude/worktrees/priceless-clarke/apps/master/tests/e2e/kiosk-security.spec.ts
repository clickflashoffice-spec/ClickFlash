...
// Kiosk tests require the full Electron kiosk app.
// Set ELECTRON_KIOSK_MODE=true in the environment when running inside Electron.
const isElectronApp = () => process.env.ELECTRON_KIOSK_MODE === 'true';

// All tests in this file require the full Electron kiosk app.
// In web-preview (Vite) context they are skipped via beforeEach.
test.describe('Kiosk Mode Security', () => {
    let page: Page;

    test.beforeAll(async ({ browser }) => {
        // Guard: don't set up page when Electron isn't running
    ...
    test.beforeEach(async () => {
        // Skip every test when not running inside the Electron kiosk app
        test.skip(!isElectronApp(), 'Kiosk security tests require the full Electron kiosk app');
    });
