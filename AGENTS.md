...
ClickFlash is an enterprise-grade automated photography concession and edge-to-cloud resort media platform. It has evolved to the **V6.1 The Autonomous Ecosystem Paradigm**. It utilizes AI for auto-culling, biometric selfie-linking, WebRTC live tracking, Rust mobile edge-computing, and dynamic yield pricing to exceed all competitors (DEI, Pomvom, Fotiqo).
...
## Code Style & Invariants (V6.1 Production Parity Rules)
...
## V6 Production Simulation Invariants
1. **Never perform destructive chaos engineering against the host environment**. Always use the ClickFlash-Prod-Sim worktree.
2. **Never execute docker-compose up prod** without the explicit presence of the .agents/V6_PROD_TAKEOVER_APPROVED gatefile.
3. Every test generation loop MUST target 100% Vitest coverage and update 	esting_matrix.md.
