# Antigravity Gemini IDE Prompt: ClickFlash Cloudflare Integration
...
The primary objective is to ** and finalize the project by integrating it with Cloudflare services for enhanced performance, scalability, and reliability. This involves a global rebranding effort and migrating existing data and services to Cloudflare D1, R2, Workers, and Pages, ensuring all applications function seamlessly within the new infrastructure.
...
### Phase 0: Global Rebranding to "Feet Hub"

1.  **Brand Identity Update:**
    *   Perform a global search and replace of "ClickFlash" with "Feet Hub" across all source code, documentation, and configuration files.
    *   Update all UI components, titles, and meta tags to reflect the new "Feet Hub" branding.
    *   Suggest and implement a new color palette suitable for "Feet Hub" (e.g., warm skin tones, elegant neutrals, or beachy aesthetics).
    *   Update all logos, favicons, and placeholder images with "Feet Hub" branded assets.
2.  **Domain & URL Updates:**
    *   Update all internal links, API endpoints, and configuration files to use "feethub.com" or its subdomains (e.g., `admin.feethub.com`, `gallery.feethub.com`).
...
The final output should be a fully functional ClickFlash ecosystem deployed on Cloudflare, with all data migrated, services running on Workers and Pages, and images stored in R2. Provide a summary report detailing the migration steps taken, any challenges encountered, and verification of successful deployment.
