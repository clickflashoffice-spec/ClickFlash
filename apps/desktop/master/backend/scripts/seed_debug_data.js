...
try {
    ...
    logger.info('Tables found:', tables.sort().join(', '));

    if (tables.includes('users')) {
        const columns = db.prepare("PRAGMA table_info(users)").all();
        logger.info('Users table columns:', columns.map(c => c.name).join(', '));
    }
    ...
    if (!user) {
        logger.info(`User ${email} missing. Creating...`);
        ...
        if (hasPassword) {
            ...
            // Fallback or error
            logger.error("No password column found in users table!");
        }
        ...
        if (insertQuery) {
            ...
            logger.info(`User restored: ${user.email} (Password: DEFAULT_PASSWORD_PLACEHOLDER)`);
        }
    } else {
    ...
    logger.info('SUCCESS: Debug data seeded.');
