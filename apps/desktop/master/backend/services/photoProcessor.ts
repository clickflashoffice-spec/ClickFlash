...
export class PhotoProcessor {
  ...
  private async checkDiskSpace(): Promise<void> {
    try {
      ...
      logger.warn("[PhotoProcessor] Disk space check failed:", error.message);
    }
  }
      ...
      await Promise.all(
        Object.keys(assets).map(async (assetKey) => {
          ...
          try {
            ...
            logger.warn(`[PhotoProcessor] Asset error ${assetKey}:`, error);
          }
        }),
      );
  ...
  public async generateWatermark(
    filepath: string,
    outputDir: string,
    watermarkConfig?: { overlayPath?: string; opacity?: number; scale?: number; position?: string }
  ): Promise<void> {
    const workerResult = await this.runWorker({
      type: "watermark",
      filepath,
  ...
  // @ts-ignore removed duplicate shutdown
  // this.pool.close();
  // }
}
