...
async function saveAlbum(album) {
  try {
    if (!album || !album.id) {
      throw new Error('Invalid album data: missing id');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      tx.onerror = () => {
        console.error('[ServiceWorker] Transaction error saving album', tx.error);
        reject(new Error(`Failed to save album: ${tx.error?.message || 'Unknown error'}`));
      };
      ...
      try {
        ...
        console.error('[ServiceWorker] Error putting album in transaction', putError);
        reject(new Error(`Failed to store album: ${putError.message || 'Unknown error'}`));
      }
    });
  } catch (error) {
    console.error('[ServiceWorker] Error in saveAlbum', error);
    throw error;
  }
}
...
async function broadcastToKiosks(message, sourceId) {
    ...
    try {
        ...
        // Broadcast to all kiosk clients except the source
        const broadcastPromises = [];
        allClients.forEach(client => {
            if (touchClients.has(client.id) && client.id !== sourceId) {
                try {
                    // Use MessageChannel for reliable delivery with error handling
                    const messageChannel = new MessageChannel();
                    const timeout = setTimeout(() => {
                        messageChannel.port1.close();
                        messageChannel.port2.close();
                        console.warn('[ServiceWorker] Broadcast timeout for client', client.id);
                    ...
                    broadcastPromises.push(
                        client.postMessage(message, [messageChannel.port2]).catch(err => {
                            clearTimeout(timeout);
                            messageChannel.port1.close();
                            console.error('[ServiceWorker] Failed to broadcast to client', client.id, err);
                        })
                    );
                } catch (postError) {
                    console.error('[ServiceWorker] Error posting message to client', client.id, postError);
                }
            }
        });
        ...
        console.error('[ServiceWorker] Error in broadcastToKiosks', error);
    }
}
        ...
        case 'NEW_ALBUM_FOR_KIOSK':
            try {
                if (!payload || !payload.id) {
                    throw new Error('Invalid album payload: missing id');
                }
                
                await saveAlbum(payload);
                
                // Broadcast to kiosks even if save fails (non-critical)
                try {
                    await broadcastToKiosks({ type: 'NEW_ALBUM_FOR_KIOSK', payload }, client.id);
                } catch (broadcastError) {
                    console.error('[ServiceWorker] Failed to broadcast album to kiosks', broadcastError);
                    // Continue - save was successful even if broadcast failed
                }
                
                if (port) port.postMessage({ success: true });
            } catch (error) {
                console.error('[ServiceWorker] Error handling NEW_ALBUM_FOR_KIOSK', error);
