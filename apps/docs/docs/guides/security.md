...
> **Classification:** Internal â€” Engineering & Operations
...
| `will-attach-webview` | PreventDefault â€” webviews are forbidden |
...
All Touch â†’ Master API requests carry an `X-Signature` header computed as:
...
- **Timestamp tolerance:** Â±5 minutes (300 seconds)
...
### 6.5 Breach Notification (Articles 33â€“34)

`logDataBreach()` records:
- `description`, `severity` (low/medium/high/critical)
- `discovered_at`, `affected_count`
- `status`: open â†’ notified â†’ resolved â†’ closed
...
- `INVALID_TOKEN` â€” JWT signature or expiry failure
- `UNAUTHORIZED_FILE_ACCESS_ATTEMPT` â€” Missing `x-service-token` for `/api/files/*`
- `NO_SESSION_OR_TOKEN` â€” Anonymous request to protected route
...
4. We follow coordinated disclosure: fix â†’ patch release â†’ public advisory.
...
- [ ] PIN brute-force lockout tested (5 attempts â†’ 15 min lockout)
...
- [ ] Crash recovery tested (3 crashes in 60s â†’ fatal error screen)
