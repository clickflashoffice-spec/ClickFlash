...
---
# ClickFlash â€” Troubleshooting Guide
...
**Alternative Fix:** Change the port in Master settings (Settings â†’ Network â†’ Port).
...
5. Restart the app â€” it will automatically recover
...
**Fix â€” Manual (IP Address):**
1. On the Master computer, open Command Prompt
2. Run: `ipconfig` and note the IPv4 address (e.g., `192.168.1.100`)
3. On the Touch Kiosk, go to Settings â†’ Manual Pairing
4. Enter the Master's IP address and port `8090`
5. Click "Connect"

**Fix â€” QR Code:**
1. On Master Portal, go to Settings â†’ Pairing â†’ Show QR Code
2. On Touch Kiosk, go to Settings â†’ Pairing â†’ Scan QR Code
...
2. If internet works, check the Cloudflare token:
   - Master Portal â†’ Settings â†’ Cloudflare â†’ Test Connection
   - If it fails, re-run the installer wizard or re-enter the token
3. Check Windows Firewall for outbound HTTPS (port 443) â€” should be allowed by default
4. If behind a corporate proxy, configure proxy settings in Master Portal â†’ Settings â†’ Network
...
**Fix:**
1. Check Touch Kiosk status bar â€” it should show "Connected to Master" or "Queued: X orders"
...
**Verify:**
1. In Master Portal, go to Orders â†’ Kiosk Orders
2. Look for orders with a "cloud offline" icon â€” these were queued and synced later
...
3. **Restart the app:**
   - Exit Master Portal completely (system tray â†’ Exit)
   - Relaunch â€” this clears any memory leaks

4. **Enable photo thumbnail cache:**
   - Master Portal â†’ Settings â†’ Performance â†’ Enable Thumbnail Cache
...
3. Enable "Pre-generate thumbnails" in Settings â†’ Performance
4. For very large albums (1000+ photos), use the "Lazy Load" option

---

## ðŸ”µ Installation Issues
...
**Fix:**
1. The installer will offer to download and install Node.js automatically â€” click "Install Node.js"
...
2. If it fails, add rules manually:
   ...
   - Click "New Rule" â†’ Program â†’ Browse to `ClickFlash-Master.exe`
   - Allow the connection â†’ Check "Private" â†’ Name it "ClickFlash Master"
...
- 8090 (Master Portal)
- 8091 (Touch Kiosk)
- 5353 (mDNS discovery â€” UDP)
- 5175 (Installer dev server â€” only during setup)
...
1. **Check the Recycle Bin / Trash:**
   ...
   - Open Money Trash from the system tray or Master Portal â†’ Tools â†’ Money Trash
   - Find the item and click "Restore"

2. **Check backups:**
   - Master Portal automatically creates daily backups
   - Go to Settings â†’ Backup â†’ Restore from Backup
...
3. **If no backups exist:**
   ...
   - Contact ClickFlash Support â€” do not delete the WAL file
...
**Fix:**
1. Master Portal â†’ Settings â†’ GDPR â†’ Data Export
...
**Fix:**
1. Master Portal â†’ Settings â†’ GDPR â†’ Data Erasure
2. Enter the customer's email address or order ID
...
- Email delivery (optional â€” emails queue when offline)
...
3. Click "Install Update" â€” the app will download, install, and restart
4. Or enable "Silent Updates" in Settings â†’ Updates to install automatically on next quit

**Manual Update:**
1. Download the latest installer from the ClickFlash website
2. Run it â€” it will detect the existing installation and update it
...
5. The Master will show a pairing request â€” click "Approve"

---

### Q: Can I use ClickFlash at multiple locations?

**A:** Yes â€” this is the multi-master fleet feature in v5.0:
...
**A:** Your data is safe:
1. SQLite uses WAL (Write-Ahead Logging) â€” transactions are durable even if the app crashes
...
4. Photos are stored on disk â€” they are not affected by app crashes

**Best Practice:** Enable automatic backups in Settings â†’ Backup â†’ Daily Backups.
...
|---------------|--------------|---------|
| **SEV 1** â€” App completely down, can't take orders | 15 minutes | Emergency hotline |
| **SEV 2** â€” Major feature broken (sync, payments) | 1 hour | Support ticket |
| **SEV 3** â€” Minor issue (UI glitch, slow performance) | 24 hours | Support ticket |
| **SEV 4** â€” Question or feature request | 72 hours | Community forum |
