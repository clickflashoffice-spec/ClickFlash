...
const BookingEditModal: React.FC<BookingEditModalProps> = ({ isOpen, onClose, onSave, bookingToEdit, photographers, sessionTypes }) => {
    ...
    const handlePhotographerChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setBooking(prev => ({ ...prev, photographerId: value ? Number(value) : undefined }));
    };
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={isNew ? "Add New Booking" : "Edit Booking"}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                    <input type="text" name="clientName" value={booking.clientName || ''} onChange={handleChange} placeholder="Client Name" required autoComplete="name" className={inputStyles} />
                    <input type="email" name="clientEmail" value={booking.clientEmail || ''} onChange={handleChange} placeholder="Client Email" required autoComplete="email" className={inputStyles} />
                </div>
                <input type="tel" name="clientPhone" value={booking.clientPhone || ''} onChange={handleChange} placeholder="Client Phone" required autoComplete="tel" className={inputStyles} />
                <div className="grid grid-cols-2 gap-4">
                    <input type="date" name="bookingDate" value={booking.bookingDate || ''} onChange={handleChange} required autoComplete="off" className={inputStyles} />
                    <input type="time" name="bookingTime" value={booking.bookingTime || ''} onChange={handleChange} required autoComplete="off" className={inputStyles} />
                </div>
                <select name="sessionId" value={booking.sessionId || ''} onChange={handleChange} required className={inputStyles}>
                    <option value="">Select Session Type</option>
                    {sessionTypes.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
                <div className="grid grid-cols-2 gap-4">
                    <select name="photographerId" value={booking.photographerId || ''} onChange={handlePhotographerChange} className={inputStyles}>
                        <option value="">Unassigned</option>
                        {photographers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                    <select name="status" value={booking.status || 'Pending'} onChange={handleChange} required className={inputStyles}>
