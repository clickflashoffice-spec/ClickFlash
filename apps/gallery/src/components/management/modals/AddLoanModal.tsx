    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Add New Loan or Capital">
            <form onSubmit={handleSubmit} className="space-y-4">
                <input type="text" value={source} onChange={e => setSource(e.target.value)} placeholder="Loan Source (e.g., Bank Loan)" autoComplete="off" className={inputStyles} required />
                <div className="grid grid-cols-2 gap-4">
                    <input type="date" value={date} onChange={e => setDate(e.target.value)} autoComplete="off" className={inputStyles} required />
                    <input type="number" value={amount} onChange={e => setAmount(e.target.value === '' ? '' : Number(e.target.value))} placeholder="Amount" autoComplete="off" className={inputStyles} required />
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <input type="number" step="0.01" value={interestRate} onChange={e => setInterestRate(e.target.value === '' ? '' : Number(e.target.value))} placeholder="Interest Rate (%)" autoComplete="off" className={inputStyles} />
                    <select value={destinationId || ''} onChange={e => setDestinationId(e.target.value || undefined)} className={inputStyles}>
                        <option value="">Company-wide (No Destination)</option>
                        {MOCK_DESTINATIONS.map(dest => <option key={dest.id} value={dest.id}>{dest.name}</option>)}
                    </select>
                </div>
                <div className="pt-4 flex justify-end space-x-3 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg">Save Loan</button>
                </div>
            </form>
        </Modal>
    );
