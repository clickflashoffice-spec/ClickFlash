...
interface ExpenseCategoryEditModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (category: Omit<ExpenseCategory, 'id'> | ExpenseCategory) => void;
    categoryToEdit: ExpenseCategory | null;
}

const ExpenseCategoryEditModal: React.FC<ExpenseCategoryEditModalProps> = ({ isOpen, onClose, onSave, categoryToEdit }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={isNew ? "Add Expense Category" : "Edit Expense Category"}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    {isNew && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">The ID will be automatically generated from the label (e.g., "My Category" becomes "MY_CATEGORY").</p>}
                </div>
