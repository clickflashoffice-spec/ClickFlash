...
const CheckoutScreen: React.FC<CheckoutScreenProps> = ({ cart, total, appliedDiscount, onBack, onCheckoutSuccess }) => {
    ...
    const handlePlaceOrder = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                await webSocketService.saveOfflineOrder(orderData);
                logger.info("Order saved to offline cache successfully", { orderId: tempId });
            } catch (offlineError) {
                logger.error("Failed to save to offline cache", offlineError instanceof Error ? offlineError : undefined, { orderId: tempId });
            ...
            try {
                ...
                logger.info("Order also saved to local DB successfully", { orderId: tempId });
            } catch (dbError) {
