...
const ObjectivesModal: React.FC<ObjectivesModalProps> = ({
  isOpen,
  onClose,
  photographer,
  onSave,
  ...
  const pastObjectives = MOCK_OBJECTIVES.filter(
    (o) => o.photographerId === photographer.id,
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Daily Objective: ${photographer.name}`}
      size="lg"
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="bg-slate-100 dark:bg-slate-700 p-4 rounded-lg min-h-[80px] flex items-center justify-center">
              <p className="text-slate-900 dark:text-slate-100 text-3xl font-bold">
                {photographer.dailyPhotoTarget
                  ? `${photographer.dailyPhotoTarget.toLocaleString()} photos`
                  : "No target set"}
              </p>
            </div>
          </div>
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-2">
              Set New Photo Target for Today
            </h3>
            <input
              type="number"
              value={newTarget}
