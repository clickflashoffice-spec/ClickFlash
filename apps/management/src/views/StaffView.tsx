...
import { Users, UserPlus, MapPin, CircleDollarSign, Clock } from 'lucide-react';
import { ShiftsHousingView } from './ShiftsHousingView';
...
export function StaffView() {
  const [activeTab, setActiveTab] = useState<'roster' | 'concession-hr'>('roster');
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            activeTab === 'roster' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800'
          }`}
        >
          <Users className="w-4 h-4" />
          Live Field Roster & Heatmap
        </button>
        <button
          onClick={() => setActiveTab('concession-hr')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            activeTab === 'concession-hr' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800'
          }`}
        >
          <Clock className="w-4 h-4" />
          Shifts, Zone Rotations, Housing & Academy
        </button>
      </div>

      {activeTab === 'concession-hr' ? (
        <ShiftsHousingView />
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 shadow-sm col-span-1 md:col-span-3">
              ...
              <div className="h-64 bg-slate-800/50 rounded-lg border border-slate-700/50 flex flex-col items-center justify-center text-slate-400 relative overflow-hidden">
                ...
                <p className="text-xs opacity-75">Live GPS coordinate telemetry active</p>
                {/* Mock heatmap blob */}
