...
export function sendError(res: Response, statusCode: number, errorType: string, message: string, code: string | null = null, details: any = null): void {
    logger.error(`[API ERROR] ${statusCode} ${errorType}: ${message}`, details ? { code, details } : { code });
    if (res.headersSent) {
        if (!res.writableEnded) {
            try { 
                res.end(); 
            } catch (e) { 
                logger.error('[ErrorHandler] Failed to end response:', e instanceof Error ? e.message : String(e));
            }
        }
        return;
    }
    ...
    try {
        ...
        if (!res.headersSent && !res.writableEnded) {
            try { 
                ...
                logger.error('[ErrorHandler] Failed to send error response:', e2 instanceof Error ? e2.message : String(e2));
            }
        }
    }
}
