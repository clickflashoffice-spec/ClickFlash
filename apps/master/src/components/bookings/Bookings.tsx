...
const StatCard: React.FC<{
  ...
  <Card className="flex items-start space-x-2.5 sm:space-x-3">
    ...
    <div className="flex-1 min-w-0">
      <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium sm:font-semibold mb-0.5 sm:mb-1 uppercase tracking-wide">
        {title}
      </p>
...
const Bookings: React.FC<BookingsProps> = ({ showToast, refreshTrigger }) => {
      ...
      <div className="flex flex-col items-center justify-center h-full min-h-[300px] sm:min-h-[400px] p-4">
        <div className="bg-red-100 dark:bg-red-900/20 rounded-full p-3 sm:p-4 mb-3 sm:mb-4">
          <svg
            ...
            className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8 text-red-500"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <p className="text-sm sm:text-base text-red-500 dark:text-red-400 mb-3 sm:mb-4 text-center max-w-md">
          {error}
        </p>
        <button
  ...
  return (
    <div className="animate-fadeIn pb-4 sm:pb-6 md:pb-8">
      ...
      {/* Today's Sessions — Priority Strip for Photographers */}
      {(() => {
        ...
        return (
          <div className="mb-4 sm:mb-5 md:mb-6 p-4 rounded-2xl bg-gradient-to-r from-blue-900/40 to-indigo-900/40 border border-blue-500/20 shadow-lg">
            <div className="flex items-center gap-2 mb-3">
              <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></span>
              <h3 className="text-xs font-black text-blue-300 uppercase tracking-widest">
                Today's Sessions — {todayBookings.length} Scheduled
              </h3>
            </div>
            <div className="flex flex-wrap gap-3">
              {todayBookings.map((b) => {
                ...
                const session = sessionTypes.find(
                  (s) => s.id === b.sessionTypeId,
                );
                const isWebBooked = !b.photographerId;
                return (
                  <button
                    key={b.id}
                    onClick={() => openModal(b)}
                    className="flex items-start gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-blue-400/30 transition-all text-left group"
                  >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        ...
                        {isWebBooked && (
                          <span className="text-[9px] font-black text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded border border-cyan-500/20">
                            WEB
                          </span>
                        )}
                      </div>
      ...
      <div className="mb-4 sm:mb-5 md:mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 sm:gap-4">
        <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-wrap w-full md:w-auto">
          <div className="flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg sm:rounded-xl shadow-sm">
            <button
              onClick={() => setViewMode("list")}
              className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-md sm:rounded-lg font-semibold text-xs sm:text-sm transition-all flex items-center gap-1.5 sm:gap-2 ${viewMode === "list" ? "bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-white" : "text-slate-500 dark:text-slate-300 hover:text-slate-700 dark:hover:text-slate-200"} focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
              title="List View"
            ...
            <button
              ...
              className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-md sm:rounded-lg font-semibold text-xs sm:text-sm transition-all flex items-center gap-1.5 sm:gap-2 ${viewMode === "calendar" ? "bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-white" : "text-slate-500 dark:text-slate-300 hover:text-slate-700 dark:hover:text-slate-200"} focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
              title="Calendar View"
          ...
          <div className="relative flex-grow md:flex-grow-0 md:w-56 lg:w-64 group">
        ...
        <div className="flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg sm:rounded-xl shadow-sm overflow-x-auto max-w-full">
          {filterOptions.map((status) => (
            <button
              key={status}
              onClick={() => setFilter(status)}
              className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-md sm:rounded-lg font-semibold text-[10px] sm:text-xs md:text-sm transition-colors whitespace-nowrap ${filter === status ? "bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-white" : "text-slate-500 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600"} focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
