...
const KioskEditModal: React.FC<KioskEditModalProps> = ({ isOpen, onClose, onSave, kioskToEdit }) => {
  ...
  const [kiosk, setKiosk] = useState<Partial<TouchKiosk>>(kioskToEdit || { name: '', id: '', status: 'Disconnected', settings: {} });
  ...
  useEffect(() => {
    if (isOpen) {
      const kioskData = kioskToEdit || { name: '', id: '', status: 'Disconnected', settings: {} };
      setKiosk(kioskData);
    }
  }, [kioskToEdit, isOpen]);
