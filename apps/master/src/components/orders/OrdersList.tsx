...
const OrdersList: React.FC<OrdersListProps> = ({
    ...
    return (
        <div className="glass-panel rounded-xl h-full flex flex-col shadow-lg overflow-hidden">
            <div className="overflow-auto custom-scrollbar flex-grow min-h-[300px]">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-sm sticky top-0 z-10 shadow-sm">
                        <tr>
                            {onSelectOrder && (
                                <th className="p-3 md:p-4 w-10 border-b border-slate-200 dark:border-slate-700">
                                    <span className="sr-only">Select</span>
                                </th>
                            )}
                            <th className="p-3 md:p-4 whitespace-nowrap text-xs md:text-sm font-semibold border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">Order ID</th>
                            <th className="p-3 md:p-4 text-xs md:text-sm font-semibold border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">Client</th>
                            <th className="p-3 md:p-4 hidden md:table-cell text-xs md:text-sm font-semibold border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">Photographer</th>
                            <th className="p-3 md:p-4 text-xs md:text-sm font-semibold border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">Status</th>
                            <th className="p-3 md:p-4 hidden sm:table-cell text-xs md:text-sm font-semibold border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">Payment</th>
                            <th className="p-3 md:p-4 text-right text-xs md:text-sm font-semibold border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">Total</th>
                            <th className="p-3 md:p-4 text-center text-xs md:text-sm font-semibold border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {orders.length === 0 ? (
                            <tr>
                                <td colSpan={onSelectOrder ? 8 : 7} className="p-12 text-center text-slate-500 dark:text-slate-400">
                            ...
                            orders.map((order) => {
                                ...
                                return (
                                    <tr
                                        key={order.id}
                                        className={`hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group ${isSelected ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                                            } ${shouldHighlight ? 'ring-1 ring-inset ring-blue-400 dark:ring-blue-500' : ''}`}
                                    >
                                        {/* Checkbox */}
                                        {onSelectOrder && (
