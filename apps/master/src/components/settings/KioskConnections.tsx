...
const KioskConnections: React.FC = () => {
    ...
    const handleSaveKiosk = async (kioskData: Omit<TouchKiosk, 'id'> | TouchKiosk) => {
        try {
            // Ensure status is valid - default to 'Disconnected' if not set or invalid
            const validStatuses = ['Active', 'Inactive', 'Maintenance', 'Connected', 'Disconnected'];
            const sanitizedData = {
                ...kioskData,
                status: (kioskData.status && validStatuses.includes(kioskData.status))
                    ? kioskData.status
                    : 'Disconnected'
            };
            ...
            if ('id' in sanitizedData && sanitizedData.id) {
                ...
                if (existingKiosk) {
                    await apiService.updateKiosk(sanitizedData.id, sanitizedData);
                } else {
                    await apiService.createKiosk(sanitizedData as TouchKiosk);
                }
            } else {
                await apiService.createKiosk(sanitizedData);
            }
                                        ...
                                        <td className="p-4 text-xs font-mono text-slate-500">
                                            <div className="flex flex-col gap-1">
                                                {kiosk.uploadFolderPath ? (
                                                    <div className="flex items-center gap-1" title={`Upload: ${kiosk.uploadFolderPath}`}>
                                                        <span className="font-bold text-slate-400">UP:</span>
                                                        <span className="truncate max-w-[200px]">{kiosk.uploadFolderPath}</span>
                                                    </div>
                                                ) : (
                                                    <span className="text-amber-500 text-[10px]">Missing Upload Path</span>
                                                )}
                                                {kiosk.ordersFolderPath ? (
                                                    <div className="flex items-center gap-1" title={`Orders: ${kiosk.ordersFolderPath}`}>
                                                        <span className="font-bold text-slate-400">OR:</span>
                                                        <span className="truncate max-w-[200px]">{kiosk.ordersFolderPath}</span>
                                                    </div>
                                                ) : (
                                                    <span className="text-amber-500 text-[10px]">Missing Orders Path</span>
                                                )}
                                            </div>
                                        </td>
