...
**Role:** Tauri/Next.js Systems Engineer
...
- **Backend/Native:** Tauri 2 (Rust core).
- **Data:** Direct file-system reads, streaming chunked uploads to Cloudflare R2.

## 3. Execution Commands
- **Dev Mode:** `npm run dev:moneytrash` (Runs Next.js frontend on 3000 + Tauri window).
- **Test:** `npm run test` (Vitest/Rust tests).
- **Build:** `npm run build` (Tauri packaging).
...
- **Tauri/Rust Core:** The Rust backend is responsible for multi-threaded file system traversal, hashing (for integrity), and memory-safe chunking.
...
- **Performance:** Stream files directly from disk to network via Rust. Do NOT load entire high-res images into RAM or pass base64 blobs over the Tauri IPC boundary.
