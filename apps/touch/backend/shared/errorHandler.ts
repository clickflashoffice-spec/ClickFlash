...
export function sendError(res: Response, statusCode: number, errorType: string, message: string, code: string | null = null, details: any = null): void {
    ...
    try {
        ...
        if (!res.headersSent && !res.writableEnded) {
            try { res.end(JSON.stringify(response)); } catch (e2) { }
        }
    }
}
...
export function sendInternalError(res: Response, error: any, context: any = null): void {
    ...
    const sanitizeMessage = (msg: string): string => {
        if (process.env.NODE_ENV !== 'development') {
            // Remove potential file paths, stack traces, and sensitive patterns
            return msg
                .replace(/\/[\w\/\.-]+/g, '[path]')
    ...
    const details = process.env.NODE_ENV === 'development'
        ? { message: sanitizeMessage(error.message), stack: error.stack, context: context }
        : null;
