...
export function captureMessage(message: string, level: string = 'info'): void {
    if (!isInitialized || !Sentry) return;
    try { Sentry.captureMessage(message, level); } catch (err) { }
}

export function setUser(user: any): void {
    if (!isInitialized || !Sentry) return;
    try { Sentry.setUser(user ? { id: user.id, email: user.email, username: user.email, role: user.role } : null); } catch (err) { }
}
