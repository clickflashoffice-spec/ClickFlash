...
const FileTransferDialog: React.FC<FileTransferDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-sm shadow-2xl w-[600px] overflow-hidden" style={{ fontFamily: 'Segoe UI, system-ui, sans-serif' }}>
        {/* Title Bar */}
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="text-sm">
            ...
            <button className="text-blue-600 hover:underline">{sourceFolder}</button>
            <span> to </span>
            <button className="text-blue-600 hover:underline">{destinationFolder}</button>
          </div>
          ...
          {showDetails && (
            <div className="space-y-1 text-sm text-gray-700">
              ...
              <div>
                ...
                <span>
                  {itemsRemaining} ({bytesRemaining > 0 ? formatBytes(bytesRemaining) : '0 bytes'})
                </span>
              </div>
            </div>
          )}
