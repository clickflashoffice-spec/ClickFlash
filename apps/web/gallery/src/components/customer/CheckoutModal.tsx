          ...
          <>
            {cart.length === 0 ? (
              ...
              <div className="max-h-[34vh] space-y-3 overflow-y-auto pr-2">
                {cart.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 rounded-2xl border border-white/5 bg-slate-900/60 p-3">
                    <img src={item.photo?.url} alt={item.name} className="h-16 w-16 rounded-xl object-cover" />
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" aria-label={`Decrease ${item.name} quantity`} onClick={() => onUpdateQuantity(item.id, item.quantity - 1)} className="h-12 w-12 rounded-xl border border-white/10 bg-slate-800 text-white">−</button>
                      <span className="w-6 text-center font-bold text-white">{item.quantity}</span>
                      <button type="button" aria-label={`Increase ${item.name} quantity`} onClick={() => onUpdateQuantity(item.id, item.quantity + 1)} className="h-12 w-12 rounded-xl border border-white/10 bg-slate-800 text-white">+</button>
                    </div>
                  </div>
                ))}
              </div>
