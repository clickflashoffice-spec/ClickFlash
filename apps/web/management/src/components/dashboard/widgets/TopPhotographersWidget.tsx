...
const TopPhotographersWidget: React.FC<TopPhotographersWidgetProps> = ({
  orders,
  photographers,
}) => {
  ...
  const photographerSales = useMemo(() => {
    ...
    orders.forEach((order) => {
      if (order.status === "Completed") {
        const pid = order.photographerId.toString();
    ...
    return photographers
      .map((p) => ({
        ...p,
        totalSales: salesMap.get(p.id.toString()) || 0,
      }))
            ...
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500 truncate">
                  {p.specialty || "Photographer"}
                </p>
              </div>
            </div>
            <p className="font-semibold font-mono text-sm text-green-600 ml-2">
