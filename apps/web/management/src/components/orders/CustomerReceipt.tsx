...
const CustomerReceipt: React.FC<CustomerReceiptProps> = ({ order }) => {
    const [settings] = useLocalStorage('customerReceiptSettings', DEFAULT_SETTINGS);
    ...
    return (
        <div className="printable-area p-8 bg-white text-black font-sans max-w-2xl mx-auto my-8 border rounded-lg shadow-lg">
            <header className="text-center pb-6 border-b-2 border-dashed">
                <img src={settings.logoUrl} alt="Company Logo" className="w-24 h-24 mx-auto mb-4 rounded-lg object-cover" />
                ...
                {settings.registrationNumber && <p className="text-xs text-gray-500 mt-1">{settings.registrationNumber}</p>}
                
                <div className="mt-6 pt-4 border-t border-gray-100">
                    <p className="text-xl font-mono font-bold">Order #{order.id}</p>
                    <div className="mt-2 flex justify-center">{renderBarcode(order.id)}</div>
                </div>
            </header>
            ...
            <section className="my-8">
                <h2 className="text-lg font-bold text-center mb-2 border-b pb-2">Online Gallery Access</h2>
                <div className="bg-gray-100 p-4 rounded-lg border border-gray-300 text-center">
                    <p className="text-sm text-gray-700 mb-3">{settings.loginInstructions}</p>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-left max-w-sm mx-auto mb-3">
                        ...
                        <div className="font-mono text-xs break-all">{settings.galleryUrl}</div>
                        
                        <div className="text-right font-semibold text-gray-600">Order ID:</div>
                    ...
                    <div className="text-xs text-gray-500 mt-2">Scan QR code (on kiosk) or use the link above.</div>
                </div>
            </section>
            
            <footer className="text-center pt-6 border-t-2 border-dashed">
                <p className="font-semibold text-gray-700">{settings.thankYouMessage}</p>
