...
import { type WatermarkSettingsType } from "../management/settings/WatermarkSettings";
...
const PrintLayout: React.FC<PrintLayoutProps> = ({ order }) => {
  ...
  const [watermarkSettings] = useLocalStorage<WatermarkSettingsType>(
    "watermarkSettings",
    {
      enabled: false,
      imageUrl: "",
    ...
    <div className="printable-area p-8 bg-white text-black font-sans max-w-[210mm] mx-auto">
      <header className="border-b-4 border-black pb-6 mb-6 flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          <p className="text-lg text-gray-600">
            Room: <strong>{order.email}</strong>
          </p>
          <p className="text-sm text-gray-500 mt-1">
            Date: {new Date(order.date).toLocaleDateString()} &bull;{" "}
      ...
      <div className="grid grid-cols-3 gap-4">
        {order.items.map((item, index) => (
          <div
            key={item.id}
            className="break-inside-avoid border-2 border-gray-800 rounded-xl overflow-hidden flex flex-col shadow-sm relative"
          >
            ...
            <div className="p-3 bg-white flex flex-col justify-between flex-grow">
              <div className="flex justify-between items-center mb-1 border-b border-gray-200 pb-2">
                <span className="font-extrabold text-lg uppercase leading-none">
                  {item.format}
                </span>
                {item.quantity === 1 && (
                  <span className="font-bold text-gray-500">x1</span>
                )}
              </div>
              <div>
                <p
                ...
                {item.name.toLowerCase().includes("ai") && (
                  <p className="text-[10px] font-bold text-white bg-purple-600 inline-block px-1.5 rounded mt-1 uppercase tracking-wide">
                    AI Edit
                  </p>
                )}
              </div>
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="text-right">
            ...
            <p className="text-gray-500 text-sm mt-1">
              Star Master OS &bull; Fulfillment System
            </p>
          </div>
        </div>
      </footer>
