...
class SyncService {
    ...
    constructor() {
        // Security: In Cloud/Public mode, we NEVER connect to a local Master IP.
        if (isPublicDomain()) {
            this.masterUrl = null;
            return;
        }
    ...
    public startSyncLoop(intervalMs = 15000) {
        // Cloud Mode: Disable Sync Loop (Rely on Realtime)
        const hostname = window.location.hostname;
        const isCloudMode = hostname !== 'localhost' && hostname !== '127.0.0.1' && !hostname.startsWith('192.168.');

        if (isCloudMode) {
            logger.info("[SyncService] Cloud Mode detected. Sync loop disabled (using Realtime).");
            return;
        }
    ...
    public async sync() {
        ...
        if (isCloudMode) return; // Strict disable
        ...
        try {
            ...
            logger.warn("[SyncService] Sync failed/skipped", {
