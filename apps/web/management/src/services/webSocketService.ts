...
class WebSocketService {
    ...
    constructor() {
        // PERMISSION FIX: Do not even attempt to use Service Workers on public domains.
        // This prevents the browser from prompting for "Local Network" permissions.
        const hostname = window.location.hostname;
        const isPublicDomain = hostname !== 'localhost' && hostname !== '127.0.0.1' && !hostname.startsWith('192.168.');

        if (isPublicDomain) {
            logger.info('[WebSocketService] Public domain detected. Service Worker Disabled.');
            return;
        }
    ...
    private registerClient = async () => {
        ...
        // PERMISSION FIX: Redundant check but good for safety
        const hostname = window.location.hostname;
        const isPublicDomain = hostname !== 'localhost' && hostname !== '127.0.0.1' && !hostname.startsWith('192.168.');
        if (isPublicDomain) {
            logger.info('[WebSocketService] Skipping registration on public domain.');
            return;
        }
    ...
    public connect(
        ...
        // Block auto-connection attempts to Service Worker in public cloud context
        // unless explicitly requested (which we can't easily know here without more config, but for now this is safe)
        // because SW message broking is designed for the Local Network context.
        if (isPublicDomain) {
            logger.info(`[WebSocketService] Public domain detected (${hostname}). Skipping Service Worker connection to prevent local network permission prompt.`, { clientType: clientInfo.type });
            return;
        }
    ...
    public getInitialAlbums(): Promise<import('../types.ts').Album[]> {
        return this.postMessageWithResponse<import('../types.ts').Album[]>({ type: 'GET_KIOSK_ALBUMS' });
    }
    ...
    public saveOfflineOrder(order: import('../types.ts').Order): Promise<void> {
        return this.postMessageWithResponse<void>({ type: 'SAVE_OFFLINE_ORDER', payload: order });
    }

    public getOfflineOrders(): Promise<import('../types.ts').Order[]> {
        return this.postMessageWithResponse<import('../types.ts').Order[]>({ type: 'GET_OFFLINE_ORDERS' });
    }
