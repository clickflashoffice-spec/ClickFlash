...
export default function BlogPage() {
    return (
        <main className="min-h-screen bg-[#0B111F] pt-24 md:pt-28 lg:pt-32 pb-12 md:pb-16">
            {/* Hero Section */}
                ...
                <div className="flex flex-wrap gap-2 md:gap-3 justify-center">
                    {categories.map((category) => (
                        <Link
                            ...
                            className="px-4 md:px-6 py-2 text-xs md:text-sm font-medium transition-all duration-300 border rounded-full bg-[#0B111F] text-white/70 border-white/20 hover:border-cyan-500 hover:text-cyan-700"
                        >
                            {category}
                        </Link>
                    ))}
            ...
            <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 md:mb-16">
                <Link href={`/blog/${blogPosts[0].slug}`} className="group block">
                    <div className="grid md:grid-cols-2 gap-6 md:gap-8 bg-white/5 rounded-2xl overflow-hidden">
                        ...
                        <div className="p-6 md:p-8 lg:p-12 flex flex-col justify-center">
                            ...
                            <h2 className="text-2xl md:text-3xl lg:text-4xl font-medium text-white mb-4 group-hover:text-cyan-700 transition-colors leading-tight">
                                {blogPosts[0].title}
                            </h2>
                            <p className="text-white/70 mb-4 md:mb-6 line-clamp-3">
                                {blogPosts[0].excerpt}
                            </p>
            ...
            <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[400px]">
                <h2 className="text-xl md:text-2xl font-medium text-white mb-6 md:mb-8">Latest Articles</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    {blogPosts.slice(1).map((post) => (
                        <Link
                            key={post.slug}
                            href={`/blog/${post.slug}`}
                            className="group flex flex-col bg-[#0B111F] overflow-hidden transition-all duration-300"
                        >
                            {/* Image Container */}
                            <div className="aspect-[4/3] relative overflow-hidden bg-white/10 mb-4 md:mb-6 rounded-xl">
                                <NextImage
                                    src={post.image}
                                    alt={post.title}
                                    fill
                            ...
                            <div className="flex flex-col">
                                ...
                                {/* Title */}
                                <h3 className="text-lg md:text-xl font-medium text-white group-hover:text-cyan-700 transition-colors leading-snug mb-3">
                                    {post.title}
                                </h3>
                                ...
                                {/* Meta */}
                                <div className="flex items-center gap-3 text-xs text-white/70 mt-auto">
            ...
            <section className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 md:mt-24 mb-8 md:mb-12">
                <div className="bg-white/5 p-8 md:p-12 text-center relative overflow-hidden border border-white/10 rounded-2xl">
                    ...
                    <p className="text-white/70 mb-6 md:mb-8 font-normal text-sm md:text-base px-2">
                    ...
                    <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto px-2">
                        <input
                            ...
                            className="flex-1 px-4 md:px-6 py-3 md:py-4 bg-[#0B111F] border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all rounded-lg text-sm md:text-base"
                        />
                        <button
                            type="submit"
