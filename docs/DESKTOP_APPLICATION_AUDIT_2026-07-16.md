...
|---|---|---|---|
| Master OS | `dist/electron/electron-main.js` | `apps/master/electron-main.ts` + `preload.ts` | `electron-main.js`, `preload.js`, `electron-new/`, four alternate builder YAML files |
| Touch Kiosk | `dist/electron/main.js` | `apps/touch/main.ts` + `preload.ts` | `main.js`, `preload.js`, `electron-package.json` |
...
| License Generator | `dist/main.js` | `apps/license-generator/src/main.ts` | no preload; renderer currently performs signing in memory |
...
Legacy implementations are not deleted until startup, IPC, native dependency, upgrade, and data-compatibility parity is evidenced.
...
| DESK-007 | P1 | Master and Touch exposed privileged IPC without validating the sender/top frame. | **Fixed:** all 13 canonical Master/Touch handlers now require the live top frame of their own BrowserWindow. Focused helper tests cover trusted and untrusted frames; payload-schema expansion remains under the shared IPC workstream. |
...
| DESK-010 | P1 | Duplicate main/preload sources and multiple builder configurations can package behavior different from reviewed source. | **Open:** canonical sources are selected above; delete alternates only after parity and upgrade evidence. |
...
|---|---:|---:|---|
| Master | 39.8.7 | 26.8.1 | five builder YAML variants; manual native unpack/resource closure |
| Touch | 39.8.7 | 26.8.1 | builder aligned; native unpack list and duplicated JS/TS shell remain |
...
- Installer complete suite passed: **54/54** across ten suites, including license Ed25519 validity/tamper/expiry/schema/machine-binding, separate payload Ed25519 trust, deterministic release signing and CLI execution, external-key custody enforcement, secret/private-material rejection, forged signatures, traversal, undeclared root/component files, tampered files, minimum-version enforcement, protected persistence, signed-bundle IPC integration, semantic config/launch schemas, multi-file commit/rollback/recovery, bounded JSON, redirect denial, private-address pinning, sender authorization, and pairing regressions.
- License Generator focused tests passed: **8/8**.
- Master Electron security helper tests passed: **2/2**; canonical Electron typecheck/build passed.
- Touch complete suite passed: **100/100** across 12 suites; canonical Electron typecheck/build passed.
- Installer production renderer and Electron builds passed; renderer bundle is 293.12 kB (90.50 kB gzip).
...
- Clean unpacked and NSIS builds completed through the canonical package commands with electron-builder 26.8.1. The final 4,927-entry ASAR passed an independent private-key-marker scan, contains the payload verifier and deliberately empty packaged trust-root module, excludes the offline release signer, and contains no payload private-key marker.
- Final `5.0.0` artifact hashes: NSIS installer SHA-256 `BBAC0EC77BBE18AC82F4C1DBB25C5749276AEDDFCC50C1752FEFFB29FA0B1266`; installed app EXE SHA-256 `EC57B84CD49B15A8C2D7B4B496F6C5EFD0137163024415FD2EFAC81024A5E29E`; `app.asar` SHA-256 `8EFC03186FDAF83CCAFF50E9BAD3DCE3C10A65F721EA733FA30D3D8B5B909A88`.
- The 99,534,880-byte NSIS installer and installed app both remain **not releasable**: Authenticode returns `NotSigned`, no production payload public key/signed release bundle exists, application acquisition/install/upgrade/repair/rollback is unfinished, and no clean-machine lifecycle evidence exists yet.
...
2. Approve and embed a separately custodied payload public key, use the completed offline release signer to produce the first authorized Master/Touch bundle, then complete acquisition plus transactional install/upgrade/repair/rollback and the explicit Windows ACL policy; deterministic signing, local signed-bundle verification, semantic application configuration, IPC schemas, SSRF controls, and OS-protected license-key persistence are complete.
3. Expand typed payload validation and structured IPC errors across Master and Touch; sender authorization is complete.
4. Consolidate Master/Touch entrypoints and builder configurations after parity tests.
5. Align Electron, builder, TypeScript, updater, and version sources across all desktop targets.
6. Prove native dependency closure in unpacked builds, then clean Windows 10/11 VMs.
7. Configure signing and signed update channels before producing production installers.
