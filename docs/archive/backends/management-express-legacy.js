...
// LEGACY: Express-based copy of Master backend for Management Hub
// Moved to archive on 2026-06-14 — Management Hub now uses Cloudflare Worker only.
// See apps/management/backend/src/server.ts for the active Worker implementation.
...
try {
    ...
    logger.warn('[Server] formidable not found, file uploads will not work. Run "npm install formidable"');
}
...
logger.info('[DEBUG] UPLOAD_DIR:', UPLOAD_DIR);
...
const JWT_SECRET = process.env.JWT_SECRET || (() => {
    // Logger not yet initialized, use console for startup warnings
    logger.warn('[Security] Using generated JWT secret. Sessions will invalidate on restart. Set JWT_SECRET in .env');
...
logger.info('');
logger.info('========================================');
logger.info('Star Master Photography OS - Master Backend Server');
logger.info('========================================');
logger.info(`[Startup] Node.js: ${process.version}`);
...
try {
    ...
    requiredDirs.forEach(dir => {
        if (!fs.existsSync(dir)) {
            try {
                ...
                logger.error(`[Fatal] Failed to create directory: ${dir}`);
                logger.error(`[Fatal] Error: ${mkdirErr.message}`);
                throw mkdirErr;
            }
        }
    });
    ...
    if (dirsCreated > 0 && process.env.NODE_ENV === 'development') {
        logger.info(`[Init] Created ${dirsCreated} directory(ies)`);
    }
    ...
    try {
        ...
        logger.info('[Init] Write permissions verified');
    } catch (permErr) {
        logger.error(`[Fatal] Write permission denied in data directory: ${DATA_DIR}`);
        logger.error(`[Fatal] Error: ${permErr.message}`);
        logger.error('[Fatal] Please check folder permissions or run as administrator');
        process.exit(1);
    }
    ...
    try {
        ...
        logger.info('[Init] Directory structure verified');
    } catch (statErr) {
        logger.warn(`[Warning] Could not verify directory stats: ${statErr.message}`);
    }
    ...
    logger.error('');
    logger.error('========================================');
    logger.error('[Fatal] Storage Access Error');
    logger.error('========================================');
    logger.error(`Error: ${err.message}`);
    logger.error(`Path: ${err.path || DATA_DIR}`);
    logger.error('');
    logger.error('Troubleshooting:');
    logger.error('  1. Run: .\\diagnose-server.bat');
    logger.error('  2. Check folder permissions');
    logger.error('  3. Ensure adequate disk space');
    logger.error('  4. See: TROUBLESHOOTING.md');
    logger.error('');
    process.exit(1);
}
...
try {
    ...
    dbManager.connect();
    logger.info(`[Init] Database connected: ${DB_FILE}`);
    ...
    setImmediate(async () => {
        try {
            ...
            logger.warn('[Init] Could not initialize default user:', initErr.message);
    ...
    logger.error(`Database: ${DB_FILE}`);
    logger.error('');
    logger.error('Troubleshooting:');
    logger.error('  1. Check if database file is locked by another process');
    logger.error('  2. Verify write permissions in data directory');
    logger.error('  3. Run: .\\diagnose-server.bat');
    logger.error('  4. See: TROUBLESHOOTING.md');
    logger.error('');
    process.exit(1);
}
...
try {
    ...
    logger.info(`[Init] Logging initialized (Level: ${process.env.LOG_LEVEL || 'INFO'})`);
} catch (logErr) {
    logger.error(`[Warning] Logging initialization failed: ${logErr.message}`);
    logger.error('[Warning] Continuing without advanced logging...');
    ...
    try {
        ...
        const saved = dbManager.transaction(() => {
            ...
            if (table === 'albums' && pathName.includes('records')) {
                logger.info(`[Debug] Checking existence for ${table} ${validData.id}: ${existing ? 'Found' : 'Not Found'}`);
            }
        ...
        const error = e instanceof Error ? e : new Error(String(e));
        logger.info('CRUD Error Debug:', error.message);
...
server.on('error', (err) => {
    ...
    if (err.code === 'EADDRINUSE') {
        ...
        logger.error('');
        logger.error('Troubleshooting:');
        logger.error(`  1. Run: .\\clear-ports.bat`);
        logger.error(`  2. Or: .\\diagnose-server.ps1 -Fix`);
        logger.error(`  3. Or manually stop the process using port ${PORT}`);
        logger.error(`  4. Or change PORT in .env file`);
        logger.error('');
        logger.error('To find the process using the port:');
        ...
        logger.error(`Error: Permission denied (Port ${PORT})`);
        logger.error('');
        logger.error('Troubleshooting:');
        logger.error('  1. Port may require administrator privileges');
        logger.error('  2. Try running as administrator');
        logger.error('  3. Or use a port number > 1024');
        logger.error('');
        ...
        logger.error('Troubleshooting:');
        logger.error('  1. Run: .\\diagnose-server.bat');
        logger.error('  2. Check server logs in: pb_data\\logs\\');
        logger.error('  3. See: TROUBLESHOOTING.md');
        logger.error('');
...
server.listen(PORT, '0.0.0.0', () => {
    logger.info('');
    logger.info('========================================');
    logger.info('[Success] Server Started Successfully');
    logger.info('========================================');
