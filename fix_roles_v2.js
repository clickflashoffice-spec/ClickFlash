...
console.log('--- Correct Role Standardization ---');
...
const getStandardRole = (raw) => {
    if (!raw) return '';
    const lower = raw.toLowerCase().trim();
    if (lower === 'ceo') return 'CEO';
    if (lower.replace(/\s/g, '') === 'teamleader') return 'Team Leader';
...
try {
    db.transaction(() => {
        ...
        users.forEach(user => {
            const raw = user.role || '';
            const standardized = getStandardRole(raw);
            if (standardized !== raw) {
        ...
        // 3. Ensure 'Admin' has manageSystemInfrastructure (Just in case)
    ...
    console.log('Final standardization complete.');
