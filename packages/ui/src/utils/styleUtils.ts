import type { ManualEdits } from '@clickflash/types';
...
export const getPhotoStyle = (edits: ManualEdits) => {
    ...
    const {
        ...
        hueRotate = 0, temperature = 0, tint: _tint = 0, whites = 0, blacks = 0,
        soften = 0, rotate = 0, straighten = 0, perspectiveX = 0, perspectiveY = 0,
        clarity = 0, dropShadow = 0, vignette: _vignette = 0
    } = edits;
...
export const isEdited = (edits: ManualEdits): boolean => {
    return Object.keys(edits).some(key => {
        const val = edits[key as keyof ManualEdits];
        const initialVal = INITIAL_EDITS[key as keyof ManualEdits];
        return val !== undefined && val !== initialVal;
    });
};
