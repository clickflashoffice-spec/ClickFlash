...
- [/] Master + Mobile Photographer flagship automatic editor and Nikon D7000 Android USB/PTP roaming capture program. Architecture, independent Kiosk/Master/Cloud receipts, shooting-spot AI, privacy-safe learning, phases, durability, image-quality, security, hardware, and release gates are defined in `docs/roadmaps/master-auto-editor-nikon-d7000-mobile.md` and `docs/roadmaps/roaming-photographer-spot-ai-kiosk-cloud.md`. The first import-only software slice now has an autolinked Expo Android module, USB attach/permission lifecycle, bounded object-delta polling, restart-aware baselines, app-private atomic JPEG/NEF import with size and SHA-256 verification, a durable SQLite capture ledger, automatic JPEG editor handoff, RAW preservation, and real tether status UI. The app is Android-only with stable package identity `com.clickflash.photographer`; TypeScript, lint, Expo introspection, module Kotlin, host-app Kotlin, and four-ABI debug APK assembly gates pass. A source-controlled Expo plugin moves Reanimated, Worklets, and Expo Modules Core native staging below the Android project to avoid Windows/Ninja path-length regeneration, and Expo packaging resolves the duplicate Worklets JNI input. Phase 0 remains open until production signing/AAB distribution and D7000 hardware, burst, detach/reconnect, restart, storage-pressure, and card-to-ledger reconciliation evidence pass.
