...
ALTER TABLE orders ADD COLUMN totalAmount REAL DEFAULT 0;
ALTER TABLE orders ADD COLUMN albumId TEXT;
ALTER TABLE orders ADD COLUMN fulfillment_status TEXT DEFAULT 'pending';
ALTER TABLE orders ADD COLUMN roomNumber TEXT;
ALTER TABLE orders ADD COLUMN orderNumber TEXT;
ALTER TABLE destinations ADD COLUMN site_code TEXT;
ALTER TABLE destinations ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_destinations_site_code_unique
  ON destinations(site_code) WHERE site_code IS NOT NULL;
